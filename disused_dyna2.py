...
def check_disused_t2(tracker_path, static_path, output_path, watchlist_path, ignore_path, days=60):
    ...
    for evse_id, info in tracker.items():
        ...
        if last_seen_dt < cutoff:
            ...
            if "iec62196T2" in connector_types:
    ...
    if not disused:
        print("No disused T2 EVSEs found older than {} days. Nothing written.".format(days))
        return
    ...
    print("{} disused T2 EVSE(s) found (not charged in {} days):".format(len(disused), days))
    for entry in sorted(disused, key=lambda x: x["evse_id"]):
